function [best_neighbor, best_total_cost, best_del, best_ind] = ts_inter_relocating(sol_info, data, param_pen, tabu_list, frequency, div)
% 路线之间重新安排顾客的位置
% 初始化
...
for i = 1:data.vnum
    ...
    for j = 1:data.vnum
        ...
        old_cost_2 = sol_info.cost(j,:); % 距离
         ...
         % 旧的总成本
        old_sum_cost_routes = sum(old_cost_2 + old_cost_1,'all');
       ...
       % 重新安排客户
        for m = 2:length(r_1)-1
            ...
            for n = 1:length(r_2)-1
                ...
                new_r_1 = r_1;
                new_r_1(new_r_1==v) = [];
                new_r_2 = [r_2(1:n), v, r_2(n+1:end)];
                
                % 计算成本
                % 1. 距离成本
                new_cost(1,1) = dist_r_1 - data.dist(r_1(m-1)+1,   r_1(m)+1) ...
                                     - data.dist(r_1(m)+1,   r_1(m+1)+1) ...
                                     + data.dist(r_1(m-1)+1, r_1(m+1)+1);
                new_cost(2,1) = dist_r_2 + data.dist(r_2(n)+1,   r_1(m)+1) ...
                                     + data.dist(r_1(m)+1, r_2(n+1)+1) ...
                                     - data.dist(r_2(n)+1, r_2(n+1)+1);
                
                % 2. 容量惩罚
                payload_1 = cap_r_1 - data.dmd(v+1);
                payload_2 = cap_r_2 + data.dmd(v+1);
                new_cost(1,2) = max([(payload_1 - data.cap),0]) * param_pen.alpha_0;
                new_cost(2,2) = max([(payload_2 - data.cap),0]) * param_pen.alpha_0;
                
                % 3. 时间惩罚
                pun_time = a_c_r_1(m-1) + data.st(r_1(m-1)+1) + data.dist(r_1(m-1)+1,r_1(m+1)+1) - z_c_r_1(m+1);
                pun_tw_r_1 = tw_f_r_1(m-1) + tw_b_r_1(m+1) + max([0, pun_time]);
                
                back_time = z_c_r_2(n+1) - data.dist(v+1,r_2(n+1)+1) - data.st(v+1);
                if back_time > data.tw(v+1,2)
                    back_time = data.tw(v+1,2);
                end
                forward_time = a_c_r_2(n) + data.st(r_2(n)+1) + data.dist(r_2(n)+1,v+1);
                if forward_time < data.tw(v+1,1)
                    forward_time = data.tw(v+1,1);
                end
                pun_time = forward_time - back_time;
                pun_tw_r_2 = tw_f_r_2(n) + tw_b_r_2(n+1) + max([0, pun_time]);
                new_cost(1,3) = pun_tw_r_1 * param_pen.beta_0;
                new_cost(2,3) = pun_tw_r_2 * param_pen.beta_0;

                % 计算减量成本 & 标记退化
                temp_decrz = old_sum_cost_routes - sum(new_cost,'all');
                if temp_decrz < 0
                ...
                % 如果小于 则接受 把最大成本的解替换掉
                temp_gen_cost = sum_ori_cost - old_sum_cost_routes + sum(new_cost,'all');
